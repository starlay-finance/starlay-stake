...
makeSuite('StakedAave. Redeem', (testEnv: TestEnv) => {
  ...
  it('User 1 stakes 50 AAVE', async () => {
    ...
    await waitForTx(await layToken.connect(staker.signer).approve(stakedToken.address, amount));
  ...
  it('User 1 activates the cooldown again, and tries to redeem a bigger amount that he has staked, receiving the balance', async () => {
    ...
    const stakedAaveBalanceBefore = (await stakedToken.balanceOf(staker.address)).toString();
    ...
    const stakedAaveBalanceAfter = (await stakedToken.balanceOf(staker.address)).toString();
