...
export const compareRewardsAtAction = async (
  ...
  const underlyingAsset = stakedLayV2.address;
  // To prevent coverage to fail, add 5 seconds per comparisson.
  await increaseTime(5);

  const rewardsBalanceBefore = BigNumber.from(
    await (await stakedLayV2.getTotalRewardsBalance(userAddress)).toString()
  );
  ...
  const assetConfiguration = assetConfig
    ...
    : {
        ...
        totalStaked: await stakedLayV2.totalSupply(),
        underlyingAsset,
      };
  await stakedLayV2.configureAssets([assetConfiguration]);

  const userBalance = await stakedLayV2.balanceOf(userAddress);
  // Get index before actions
  const userIndexBefore = await getUserIndex(stakedLayV2, userAddress, underlyingAsset);
  ...
  const userIndexAfter = await getUserIndex(stakedLayV2, userAddress, underlyingAsset);
...
export const compareRewardsAtTransfer = async (
  ...
  const underlyingAsset = stakedLayV2.address;
  const fromSavedBalance = await stakedLayV2.balanceOf(fromAddress);
  const toSavedBalance = await stakedLayV2.balanceOf(toAddress);
  const fromSavedRewards = BigNumber.from(
    await (await stakedLayV2.getTotalRewardsBalance(fromAddress)).toString()
  );
  const toSavedRewards = BigNumber.from(
    await (await stakedLayV2.getTotalRewardsBalance(toAddress)).toString()
  );
  // Get index before actions
  const fromIndexBefore = await getUserIndex(stakedLayV2, fromAddress, underlyingAsset);
  const toIndexBefore = await getUserIndex(stakedLayV2, toAddress, underlyingAsset);

  // Load actions that can or not update the user index
  const actions = () => [stakedLayV2.connect(from.signer).transfer(toAddress, amount)];

  // Fire reward comparator
  await compareRewardsAtAction(stakedLayV2, fromAddress, actions, fromShouldReward, assetConfig);

  // Check rewards after transfer

  // Get index after actions
  const fromIndexAfter = await getUserIndex(stakedLayV2, fromAddress, underlyingAsset);
  const toIndexAfter = await getUserIndex(stakedLayV2, toAddress, underlyingAsset);
  ...
  if (fromAddress === toAddress) {
    ...
    const fromNewBalance = await (await stakedLayV2.balanceOf(fromAddress)).toString();
    const toNewBalance = await (await stakedLayV2.balanceOf(toAddress)).toString();
