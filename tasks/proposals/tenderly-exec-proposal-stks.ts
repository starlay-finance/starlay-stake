...
task('exec-stks:tenderly', 'Execute staking extension proposal at Tenderly fork')
  ...
  .setAction(async ({ defender }, localBRE: any) => {
    ...
    console.log('- StakedLayV2 Distribution End');
    ...
    const rewardAndRedeem = async (
      ...
      try {
        ...
        const tokenBalanceBefore = await layToken.balanceOf(await signer.getAddress());
        await waitForTx(
          await stakedToken.claimRewards(await signer.getAddress(), MAX_UINT_AMOUNT, {
            gasLimit: 3000000,
          })
        );
        const tokenBalanceAfter = await layToken.balanceOf(await signer.getAddress());
        const rewards = tokenBalanceAfter.sub(tokenBalanceBefore);
