...
task('no-exec-stks:tenderly', 'Execute staking extension proposal at Tenderly fork')
  ...
  .setAction(async ({ defender }, localBRE: any) => {
    ...
    const STAKED_RAY = '0x4da27a545c0c5B758a6BA100e3a049001de870f5';
    ...
    const stakedTokenV2 = StakedLayV2__factory.connect(STAKED_RAY, proposer);
    const bptStakeV2 = StakedLayV2__factory.connect(STK_BPT_STAKE, proposer);

    // Transfer enough LAY to proposer
    await (await layToken.transfer(await proposer.getAddress(), parseEther('100'))).wait();
    ...
    console.log('- Staked Lay v2 Distribution End');
    console.log('  - Distribution End', await (await stakedTokenV2.DISTRIBUTION_END()).toString());
    console.log('  - Revision', await (await stakedTokenV2.REVISION()).toString());
    console.log('  - Name', await stakedTokenV2.name());
    console.log('  - Symbol', await stakedTokenV2.symbol());
    console.log('  - Decimals', await stakedTokenV2.decimals());
    ...
    const rewardAndRedeem = async (
      ...
      try {
        ...
        const layBalanceBefore = await layToken.balanceOf(await signer.getAddress());
        await waitForTx(
          await stakedToken.claimRewards(await signer.getAddress(), MAX_UINT_AMOUNT, {
            gasLimit: 3000000,
          })
        );
        const layBalanceAfter = await layToken.balanceOf(await signer.getAddress());
        const rewards = layBalanceAfter.sub(layBalanceBefore);
        console.log(`- Claimed Lay:`, await formatEther(rewards));
    ...
    for (let x = 0; x < 3; x++) {
      for (let y = 0; y < 10; y++) {
        await rewardAndRedeem(top10stkTokenHolders[y], stakedTokenV2.address, 'lay');
